...
function initializeAll(){
    ...
    setInitial()
        ...
        .then(function(){
            return setTagsWithSelection($("#cField"), 'work', $workID);
        })
...
function setInitial(){
    ...
    $('#frm-creative-work').on('submit', function(e){
        tinymce.get('ta-content').save();
...
function getData()
{
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url: '/shd/creative-works/get-story-data',
            type: 'POST',
            dataType: 'json',
            data: { slug: slug },
            headers: {
...
function setData(response){
    ...
    $workID = response.story.id;
    $('#title').val(response.story.title);
    // $('#ta-content').html(@json(response.content));
    $('#ta-content').html(response.content);
    if(response.story.published_status == 1){
