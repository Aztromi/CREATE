...
use App\Models\LogsProfileStateChange;
use App\Models\Slug;
...
class ProfileController extends Controller
{
    ...
    {
        $profile = UProfile::with('latestSlug', 'uindie.expertises', 'addressLatest', 'websites', 'socials', 'jobTitleFirst', 'sectors', 'user', 'emails')
                    ->where('user_id', Auth::user()->id)->first();

        return view('dashboard.content.account')
    ...
    public function editProfile($id = null)
    {
        ...
        return view('dashboard.content.account-form')
    ...
    // TRANSFERED TO SHARED
    // public function getProfileData(Request $request)
    // {
    //     // In Admin Side, assign proID before form Show
    //     // if($session()->has('proID'))
    //     // {
    //     //     $profileID = $session('proID');
    //     // }
    //     // else
    //     // {
    ...
    //     if(!$profile)
    //     {
    //         return response()->json($profile, 422);
    //     }
    ...
    //         if($request->input('same_rep_owner') != '1')
    //         {
