...
class ArticleController extends Controller
{
    ...
    {
        $articles = Article::with('asset', 'latestSlug')->orderBy('updated_at', 'desc')->get();
    ...
    public function showEditForm($id = null)
    {
        ...
        $article = Article::with('asset')->where('id', $id)->first();

        if(!$article){
            return redirect()->route('admin.article-list');
        }
        
        $masthead_name = optional($article->asset)->path;
        $masthead = $masthead_name ? '/folder_articles/' . $masthead_name : null;
